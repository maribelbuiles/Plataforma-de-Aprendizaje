...
if st.session_state['cedula'] is None:
        ...
        with tabs[8]:
            ...
            if submit_eval:
                ...
                if score >= 80:
                    ...
                    st.success(f"¡APROBADO CON {score}%! Ya puedes visualizar y descargar tu certificado en la siguiente pestaña.")
                    st.balloons()
                else:
                    st.session_state['aprobado_m1'] = False
                    st.error(f"Puntaje insuficiente: {score}%. Necesitas 80% para aprobar. Repasa el material.")

        # Pestaña 10: Certificado Módulo 1
        with tabs[9]:
    ...
    # --- CONTENIDO MÓDULO 2 ---
        ...
        tabs = st.tabs(["📖 Contenido Informativo", "📝 Evaluación", "🎓 Certificado"])
        with tabs[0]:
