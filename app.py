...
if st.session_state['cedula'] is None:
    ...
    st.sidebar.markdown("---")
    # Selector interactivo para enunciar las opciones sin generar los componentes en el sidebar
    seccion = st.sidebar.radio("📍 Sección del Módulo", [
    ...
    if modulo == "Módulo 1: Equipo de Canastas Aptas":
        ...
        if seccion == "📖 Contenido Informativo":
            tabs = st.tabs([
                "🕒 Cronología", 
                "🔍 Partes", 
                "📐 Dimensiones", 
            ...
            if submit_eval:
                ...
                if score >= 80:
                    ...
                    st.success(f"¡APROBADO CON {score}%! Ya puedes visualizar y descargar tu certificado en la sección correspondiente.")
                    st.balloons()
                else:
                    st.session_state['aprobado_m1'] = False
                    st.error(f"Puntaje insuficiente: {score}%. Necesitas 80% para aprobar. Repasa el material.")

        elif seccion == "🎓 Certificado":
